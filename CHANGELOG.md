...
川农教学身份这一版只保留三种身份：超级管理员、教师、学生。

- **NEW**: 系统管理增加「用户管理」。超级管理员可以看全部已注册用户，并在表里把身份改成学生、教师或超级管理员。
- **NEW**: 超级管理员可以进入并管理任意工作空间及其下的课。教师仍只能管理自己的空间，学生仍只能进入被邀请的课。
- **IMPROVED**: 成员管理只保留这门课的名册。空间主人显示为教师；若该账号已是超级管理员，则显示超级管理员。被邀请的人显示为学生。邀请和分享链接不再选择角色。
- **REMOVED**: 成员页的角色下拉、权限说明和「了解 RBAC」入口。平台身份不在成员页修改。
